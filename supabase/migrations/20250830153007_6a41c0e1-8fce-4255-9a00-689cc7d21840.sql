-- Tables pour le système QVT Box poétique (version corrigée)
...
-- Insérer quelques rituels par défaut (seulement s'ils n'existent pas)
INSERT INTO public.rituals (title, description, bubble_type, duration_minutes, instructions) 
SELECT * FROM (VALUES
  ('Respiration consciente', 'Prenez 3 minutes pour respirer profondément et vous recentrer', 'soin'::bubble_type, 3, '{"steps": ["Asseyez-vous confortablement", "Fermez les yeux", "Respirez profondément 10 fois", "Focalisez-vous sur votre respiration"]}'),
  ('Pause gratitude', 'Notez mentalement 3 choses pour lesquelles vous êtes reconnaissant', 'inspiration'::bubble_type, 2, '{"steps": ["Pensez à votre journée", "Identifiez 3 moments positifs", "Ressentez la gratitude", "Souriez intérieurement"]}'),
  ('Micro-étirement', 'Quelques étirements doux pour détendre votre corps', 'soin'::bubble_type, 5, '{"steps": ["Étirez vos bras vers le ciel", "Roulez vos épaules", "Penchez la tête doucement", "Respirez pendant chaque mouvement"]}'),
  ('Check-in équipe', 'Prenez des nouvelles sincères d''un collègue', 'connexion'::bubble_type, 4, '{"steps": ["Choisissez un collègue", "Demandez comment ça va vraiment", "Écoutez activement", "Partagez votre propre ressenti"]}')
) AS v(title, description, bubble_type, duration_minutes, instructions)
WHERE NOT EXISTS (SELECT 1 FROM public.rituals WHERE title = v.title);
