-- Tables pour le système QVT Box poétique

-- Enum pour les types de bulles émotionnelles
CREATE TYPE bubble_type AS ENUM ('Focus', 'Cohésion', 'Récupération', 'Gratitude');

-- Enum pour les rôles d'utilisateur
CREATE TYPE user_role AS ENUM ('salarié', 'manager', 'rh', 'admin');

-- Enum pour les parcours utilisateur
...
CREATE TABLE public.daily_bubbles (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  date DATE NOT NULL DEFAULT CURRENT_DATE,
...
INSERT INTO public.rituals (title, description, bubble_type, duration_minutes, instructions) VALUES
('Respiration consciente', 'Prenez 3 minutes pour respirer profondément et vous recentrer', 'Focus', 3, '{"steps": ["Asseyez-vous confortablement", "Fermez les yeux", "Respirez profondément 10 fois", "Focalisez-vous sur votre respiration"]}'),
('Pause gratitude', 'Notez mentalement 3 choses pour lesquelles vous êtes reconnaissant', 'Gratitude', 2, '{"steps": ["Pensez à votre journée", "Identifiez 3 moments positifs", "Ressentez la gratitude", "Souriez intérieurement"]}'),
('Micro-étirement', 'Quelques étirements doux pour détendre votre corps', 'Récupération', 5, '{"steps": ["Étirez vos bras vers le ciel", "Roulez vos épaules", "Penchez la tête doucement", "Respirez pendant chaque mouvement"]}'),
('Check-in équipe', 'Prenez des nouvelles sincères d''un collègue', 'Cohésion', 4, '{"steps": ["Choisissez un collègue", "Demandez comment ça va vraiment", "Écoutez activement", "Partagez votre propre ressenti"]}');
...
BEGIN
  ...
  INSERT INTO daily_bubbles (user_id, date, bubble_type, intensity, message, ritual_suggestion) VALUES
    ...
    (NEW.user_id, NEW.date, 'Cohésion', cohesion_intensity,
    ...
    (NEW.user_id, NEW.date, 'Récupération', recuperation_intensity,
    ...
    (NEW.user_id, NEW.date, 'Gratitude', gratitude_intensity,
...
-- Trigger pour générer automatiquement les bulles
